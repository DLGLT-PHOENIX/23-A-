...
fp32 PidCalculate(pid_parameter_t *pid, fp32 SetValue, fp32 ActualValue)
{
    ...
    {
        //pid_clear(pid); 浮点数0与0比较时不知道为什么会进这里
    }
...
{
    spid->SetValue =1 + PidCalculate(ppid, setPosition, actual_position); //速度环设定值由位置环处理
    return PidCalculate(spid, spid->SetValue, actual_speed);              //电机输出量
}
...
**************************************************************************/
float Incremental_PID(pid_parameter_t *PID,float actualval,float setval)
{	
	 PID->error = setval - actualval;                                                          //计算偏差
	 PID->out += PID->Kp * (PID->error - PID->LastError)
	 + PID->Ki * PID->error + PID->Kd * (PID->error - 2 * PID->LastError + PID->LastLastError);  //增量式PID控制器
	 PID->LastLastError=PID->LastError;                                                       //保存上次偏差
	 PID->LastError=PID->error;	                                                               //保存本次偏差 
//	 if( PID->out> PID->Result_max)       PID->out= PID->Result_max;                   //输出限幅
//	 else if( PID->out< PID->Result_min)  PID->out = PID->Result_min;
	 return PID->out;  	
}
